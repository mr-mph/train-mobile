...
def test_attach_wraps_read_latest_and_publishes_to_hub() -> None:
    ...
    with patch("lelab.camera_stream.camera_hub") as hub:
        ...
        out = cam.read_latest()
        assert out is frame
        assert broker.latest("wrist") is frame
        hub.push_relay_frame.assert_called()
        ...
        cam.read_latest(max_age_ms=100)
        assert cam.read_latest.call_count >= 1


def test_camera_index_from_live_index_attr() -> None:
    from lelab.frame_broker import _camera_device_index

    cam = SimpleNamespace(index=2, config=SimpleNamespace(index_or_path=9))
